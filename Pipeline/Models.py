...
class MTL(nn.Module):
    def __init__(self, state="M1"):
        ...
        self.macular_edema_classifier = nn.Sequential(nn.Linear(1024, 512), nn.ReLU(), nn.Linear(512, 3))
    ...
    def fit(self, train_dl, optimizer, scheduler, criterion_t0, criterion_t1, tasks, epochs, Rx, Ry):
        self.optimizer = optimizer
        self.scheduler = scheduler
        self.criterion_t0 = criterion_t0
        self.criterion_t1 = criterion_t1
    ...
    def fit_iter(self, train_dl, Rx, Ry):
        ...
        for i, tupel_train in enumerate(train_dl):
            ...
            if self.state == 'M1':
                loss0 = self.criterion_t0(retinopathy_pred, retinopathy_label.to(device).to(torch.int64)).to(
                    torch.float64) * 10
                loss1 = self.criterion_t1(macular_edema_pred, macular_edema_label.to(device).to(torch.int64)).to(
                    torch.float64) * 10
                ...
                loss0 = loss0CE*0.7 + loss0KL*0.3
                loss1 = loss1CE*0.7 + loss1KL*0.3
            elif self.state =="M3":
                loss0 = self.criterion_t0(F.log_softmax(retinopathy_pred.double(), dim=-1),
                                       F.softmax(retinopathy_label.to(device).double(), dim=-1)).double()
                loss1 = self.criterion_t1(F.log_softmax(macular_edema_pred.double(), dim=-1),
        ...
        if self.state == 'M1':
            acc0 = accuracy0 * 100 / 413
            acc1 = accuracy1 * 100 / 413
            print("\nTotal Loss: {}\nLoss0: {}  Accuracy0: {}\nLoss1: {}  Accuracy1: {}\nLoss2: {}\nLoss3: {}".format(
                train_loss, loss0sum, acc0, loss1sum, acc1, loss2sum, loss3sum))
            self.acc0_hist.append(acc0)
