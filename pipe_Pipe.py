...
class Pipe():
    ...
    def fit_M1(self,tasks,epochs):
        ...
        self.M1.fit(self.M1_train_dl,self.M1_optimizer,self.M1_scheduler,self.M1_criterion,tasks,epochs,self.Rx,self.Ry)
        self.M1.load_state_dict(torch.load("./M1_weights"+str(tasks)+".pt"))
        self.M1.eval()
        data=pd.DataFrame()
        z = []
        for i, (imgs, retinopathy_label, macular_edema_label, fovea_center_labels, optical_disk_labels) in enumerate(self.M1_train_ds):
